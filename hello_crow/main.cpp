...
using namespace crow::mustache;//tu use mustache
...
int main(int argc, char* argv[]){
  ...
  crow::SimpleApp app;
  set_base(".");//withut set_base(location) mustache doesnt work

  mongocxx::instance inst{};
  string mongoConnect = std::string(getenv("MONGODB_URI"));
  mongocxx::client conn(mongocxx::uri{mongoConnect});
  auto collection = conn["heroku_2kzc7q37"]["contacts"];
  ...
  CROW_ROUTE(app, "/about")
    ...
    CROW_ROUTE(app, "/add/<int>/<int>")//The order of the routes is importante, it will stop the time it finds the first one
    ...
    ([](const request &req, response &res){
      ...
      ostringstream os;
      os << "Hello " << (firstname? firstname: "") << " " << (lastname? lastname: "") << endl;
      res.set_header("Content-Type", "text/plain");
      res.write(os.str());
      res.end();
    });
